...
of the required version is running it imports only `magick_builder.usage`, which
...
CONDA_ENVIRONMENT = "install-imagemagick"
...
RESOLVED_GUARD = "MAGICK_BUILD_INTERPRETER_RESOLVED"
...
def resolve_conda_interpreter(roots: list[str]) -> str | None:
    ...
    for root in roots:
        ...
        return python
    root = roots[0]
    question = "Create conda environment '%s' (python=%s)? [y/N]: " % (
        CONDA_ENVIRONMENT,
...
def main(argv: list[str]) -> int:
    ...
    from magick_builder.usage import metadata_response
    ...
    from magick_builder.main import main as run_build
