...
instead, a wall-clock and elapsed stamp on every record.
    ...
    Building libpng - version 1.6.58
    ================================
    [INFO] Downloading https://example.test/libpng-1.6.58.tar.gz as 'libpng-1.6.58.tar.gz'.
    $ make -j24
    [DONE] libpng 1.6.58 built in 8s
...
class Logger:
    ...
    def blank(self) -> None:
        """Separate two groups of records; never stacks into a double gap."""
        self._blank()
    ...
    def run(
        ...
        *,
        ...
        records it either way. `shown` replaces the terminal text only, so a
        command carrying a long file list can print one readable line while
        the log keeps every argument.
        """
        self._command(message, arguments, quiet=quiet, shown=shown)
    ...
    def magick(self, message: str) -> None:
        """A line reported by the magick binary itself, trailing space trimmed."""
        self.line("MAGICK", message.rstrip())
