...
variables. A Conda environment (the launcher runs this build inside one) reaches
a native build through `LD_LIBRARY_PATH`, `CMAKE_PREFIX_PATH`,
`PKG_CONFIG_PATH`, `CONDA_BUILD_SYSROOT`, `SSL_CERT_FILE` and activation-set
`CC`/`CFLAGS`/`LDFLAGS`; starting from a curated base makes that leakage
structurally impossible instead of enumerably unlikely.
"""
...
class Runner:
    ...
    def _run_logged(
        ...
        *,
        ...
        directly; `execute` is this plus fail-on-non-zero. `display` replaces
        the echoed argv on the terminal only, for commands whose arguments are
        long file lists; the build log still records the complete command.
        """
        if not arguments:
            raise BuildError("run_logged() called without a command.")
        command_display = shellquote.join(list(arguments))
        self.logger.run(command_display, arguments=arguments, quiet=quiet, shown=display)
    ...
    def execute(
        ...
        *,
        ...
        command_display = display or shellquote.join(list(arguments))
    ...
    def capture(
        ...
        *,
        ...
        Probes decide optional behavior. Turning a missing optional feature
        into an exception would abort builds the recipe deliberately wrote to
        continue, so a missing executable and a non-zero exit are reported the
        same way: through the returned status.
        """
        try:
            with managed_process(
                subprocess.Popen(
